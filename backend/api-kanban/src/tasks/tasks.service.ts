...
export class TasksService {
  ...
  update(id: string, data: Partial<Task>) {
    return this.model.findByIdAndUpdate(id, data, { new: true }).then(doc => {
      if (doc) this.events.emitToBoard(String(doc.boardId), 'task.updated', doc);
      return doc;
    });
  }
  ...
  move(id: string, dto: MoveTaskDto) {
    return this.model.findByIdAndUpdate(id, dto, { new: true }).then(doc => {
      if (doc) this.events.emitToBoard(String(doc.boardId), 'task.moved', doc);
      return doc;
    });
  }
  ...
  remove(id: string) {
    return this.model.findByIdAndDelete(id).then(doc => {
      if (doc) this.events.emitToBoard(String(doc.boardId), 'task.deleted', { id, columnId: doc.columnId });
      return doc;
    });
  }
}
